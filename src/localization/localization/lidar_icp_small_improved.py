...
class LidarICP(Node):
    def __init__(self):
        ...
        # Small but meaningful robustness additions
        self.min_points_after_voxel = 30
        self.max_icp_translation_correction = 0.15  # m
        self.max_icp_yaw_correction = 0.15          # rad (~8.6 deg)
        self.icp_correction_alpha = 0.6             # damp map->odom updates
        self.use_odom_covariance_gating = True
        self.min_sigma_xy = 0.05
        self.min_sigma_yaw = np.deg2rad(2.0)
        self.covariance_gate_scale = 2.5
    ...
    def get_odom_sigmas(self):
        """Extract rough planar 1-sigma uncertainties from nav_msgs/Odometry covariance."""
        sigma_xy = self.min_sigma_xy
        sigma_yaw = self.min_sigma_yaw
        ...
        try:
            ...
            sigma_xy = max(np.sqrt(max(var_x, var_y)), self.min_sigma_xy)
            sigma_yaw = max(np.sqrt(var_yaw), self.min_sigma_yaw)
        except Exception:
            pass

        return sigma_xy, sigma_yaw

    def extract_planar_delta(self, T):
        dx = float(T[0, 3])
        dy = float(T[1, 3])
        dyaw = float(np.arctan2(T[1, 0], T[0, 0]))
        return dx, dy, self.wrap_to_pi(dyaw)

    def make_planar_transform(self, dx, dy, yaw):
        c = np.cos(yaw)
        s = np.sin(yaw)
        return np.array([
            [c, -s, 0.0, dx],
    ...
    def gate_and_damp_icp_correction(self, T_icp):
        """Reject implausible ICP corrections and damp accepted ones."""
        dx, dy, dyaw = self.extract_planar_delta(T_icp)
        trans = np.hypot(dx, dy)

        sigma_xy, sigma_yaw = self.get_odom_sigmas()
        max_trans = self.max_icp_translation_correction
        max_yaw = self.max_icp_yaw_correction

        if self.use_odom_covariance_gating:
            max_trans = max(max_trans, self.covariance_gate_scale * sigma_xy)
            max_yaw = max(max_yaw, self.covariance_gate_scale * sigma_yaw)

        if trans > max_trans or abs(dyaw) > max_yaw:
            self.get_logger().warn(
                f"Rejecting ICP correction: trans={trans:.3f} m, dyaw={np.degrees(dyaw):.2f} deg, "
                f"limits=({max_trans:.3f} m, {np.degrees(max_yaw):.2f} deg)"
            )
            return None

        alpha = float(np.clip(self.icp_correction_alpha, 0.0, 1.0))
        return self.make_planar_transform(alpha * dx, alpha * dy, alpha * dyaw)
    ...
    def scan_callback(self, msg):
        ...
        if len(current_pcd.points) < self.min_points_after_voxel:
            self.get_logger().warn("Too few points after voxel filtering, skipping ICP")
            return
        ...
        if len(local_map.points) < self.min_points_after_voxel:
            self.get_logger().warn("Local map too small for reliable ICP, skipping")
            return
        ...
        T_correction = self.gate_and_damp_icp_correction(T_icp)
        if T_correction is None:
            return

        self.T_map_to_odom = T_correction @ self.T_map_to_odom

        dx_corr, dy_corr, dyaw_corr = self.extract_planar_delta(T_correction)
        self.get_logger().info(
            f"icp fitness: {icp_result.fitness:.2f}, rmse: {icp_result.inlier_rmse:.3f}, "
            f"applied correction: ({dx_corr:.3f}, {dy_corr:.3f}, {np.degrees(dyaw_corr):.2f} deg)"
        )

        current_pcd.transform(T_correction)
